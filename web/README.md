...
- The network and place overlays are static files under `public/data`, one per
  city, generated by `../tools/extract-transit.py` and `../tools/extract-places.py`.
  Fetched once per city and filtered in memory when a toggle changes.
...
- Responses are raw MOTIS JSON (~4 MB, ~350 KB compressed over the wire by
  ...
  further.
- Overlay data ships with the frontend rather than being served by the routing
  server, so refreshing it means re-running the extract scripts and pushing.
